...
`pruneCache` は、指定した時刻以降に Rotion が使っていないものを削除します。
...
`before` には、使っているものがそれより古くならない時刻を選びます。
全体をビルドした直後なら、ビルドを始めた時刻で足ります。ビルドは表示するものをすべて読むからです。
`next build` の前に時刻を記録しておき、ビルドの後に削除するスクリプトにできます。
サーバーでは、Rotion を呼ばずにページを配信し続ける期間より長い期間を使います。たとえば、ページの `revalidate` に余裕を足した期間です。
