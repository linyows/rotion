...
参照されなくなったファイルは、削除するまで残ります。[使われなくなったファイルの削除](#removing-unused-files)を参照してください。
...
## 使われなくなったファイルの削除 [#removing-unused-files]

キャッシュとダウンロードしたファイルは増える一方です。
Notion で削除したページや、差し替えた画像のファイルも残り続けます。
`rotion prune` コマンドは、指定した期間 Rotion が使っていないものを削除します。

```bash
npx rotion prune --unused-for 7d            # s、m、h、d のいずれか
npx rotion prune --before 2026-09-20T00:00:00Z
npx rotion prune --unused-for 7d --dry-run  # 一覧を出すだけ
```

削除したパスと件数を出力します。
対象のディレクトリは、ビルドと同じ環境変数（`ROTION_CACHEDIR`、`ROTION_DOCROOT`、`ROTION_IMAGEDIR`、`ROTION_FILEDIR`）から決まります。
コードからは `pruneCache` で同じことができます。
...
Rotion は、使ったファイルのアクセス時刻を更新して印を付けます。
キャッシュファイルは読み書きしたとき、ダウンロードしたファイルは保存したときとディスクで見つけたときに印を付けます。
キャッシュした結果を読んだときは、その結果が参照している画像とファイルにも印を付けます。
そのため、キャッシュから表示したページの画像も残ります。
キャッシュの有効期限の判定に使う更新時刻は変えません。

- 削除するのは、Rotion が付ける名前のファイルだけです。`notion.` で始まるキャッシュファイルと、名前がハッシュで終わるダウンロード（`block-<id>-<hash>.webp`）が対象です。サイト自体の画像など、`public/images` にあるほかのファイルは残します。
- ダウンロードしたファイルとその変換結果（画像の WebP、HEIC 画像の PNG）はまとめて扱い、どれも使われていないときにだけ削除します。
- 書き込みに失敗して残った一時ファイルも削除します。ロックファイルには触れません。
- `dryRun: true` を渡すと、削除せずに一覧だけを返します。

期間は、使っているものがそれより古くならないように選びます。
全体のビルドは表示するものすべてに印を付けるので、ビルドの直後であれば、ビルドにかかる時間より長い期間を指定すれば安全です。
...
サーバーでは、Rotion を呼ばずにページを配信し続ける期間より長い期間を指定します。
たとえば、ページの `revalidate` に余裕を足した期間です。
定期的に実行するジョブから呼ぶとよいでしょう。
