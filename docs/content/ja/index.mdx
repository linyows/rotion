...
description: Rotion は Notion のページとデータベースを React コンポーネントで描画し、静的サイトとしてビルドするか、サーバーで描画します。
---
...
>
  <Lede>
    ...
    ギャラリー、テーブル、カレンダー、各種ブロックはスタイル付きで用意されていて、サイトは静的 HTML としてビルドすることも、サーバーで描画することもできます。
    ここに表示しているページとデータベースは、どれもこのページのビルド時に Notion から取得し、Rotion で描画したものです。
  </Lede>
  <Actions>
